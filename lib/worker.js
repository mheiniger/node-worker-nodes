...
class Worker extends EventEmitter {
    constructor({ srcFilePath, maxTasks, maxMessageSize, endurance, stopTimeout, workerStopShift, initMethod, initArgs }) {
        super();

        this.id = workerSerial.nextValue();
        this.calls = new Map();
        this.tasksStarted = workerStopShift || 0;
        this.maxTasks = maxTasks;
        this.endurance = endurance;
        this.isTerminating = false;
        this.isProcessAlive = false;

        const process = this.process = new WorkerProcess(srcFilePath, { maxMessageSize, stopTimeout, initMethod, initArgs });
