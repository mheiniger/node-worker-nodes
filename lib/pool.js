...
class WorkerNodes extends EventEmitter {
        ...
        this.isReady = new Promise(resolve => {
            ...
            if (this.options.autoStart) {
                if (this.options.lazyStart) {
                    ...
                    let currentStopShift = 0;
                    while (this.canStartWorker()) {
                        this.startWorker(currentStopShift);
                        currentStopShift = currentStopShift + this.options.workerStopShift
                    }
                }
            }
     ...
     *
     * @param {Number} workerStopShift
     * @private
     */
    startWorker(workerStopShift = 0) {
        let workerOptions = Object.assign({workerStopShift}, this.workerOptions);
